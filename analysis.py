...
def describe(series: Sequence[float]) -> SeriesStats:
    ...
    return SeriesStats(len(series), low, high, average, sqrt(variance), ((high - low) / average * 100) if average else 0)
...
def latency_qos(samples: Iterable[dict], field: str = "latency") -> dict:
    ...
    return {"average": describe(series).average, "p50": percentile(series, 50), "p90": percentile(series, 90), "p95": percentile(series, 95), "p99": percentile(series, 99), "p999": percentile(series, 99.9), "p9999": percentile(series, 99.99)}


def detect_knee(series: Sequence[float], window: int = 3, threshold_percent: float = 20) -> KneePoint | None:
    ...
    for index in range(window, len(series) - window + 1):
        ...
        if drop >= threshold_percent:
            consistency = sum(1 for value in series[index:index + window] if value <= baseline * (1 - threshold_percent / 100)) / window
...
def temperature_correlation(samples: Sequence[dict]) -> float:
    ...
    pairs = [(float(s["bandwidth"]), float(s["temperature"])) for s in samples if isinstance(s.get("bandwidth"), (int, float)) and isinstance(s.get("temperature"), (int, float))]
    if len(pairs) < 3:
        return 0.0
    xs, ys = zip(*pairs)
...
def analyse(samples: Sequence[dict]) -> dict:
    ...
    return {"bandwidth": describe(bandwidth).to_dict(), "iops": describe(iops).to_dict(), "latency": latency_qos(samples), "knee": knee.to_dict() if knee else None, "stability": stability_grade(describe(bandwidth)), "thermal_correlation": temperature_correlation(samples), "median_bandwidth": median(bandwidth) if bandwidth else 0}
