...
def record(con: sqlite3.Connection, category: str, action: str, subject: str, details: dict | None = None) -> int:
    cursor = con.execute("INSERT INTO audit_events (created_at,category,action,subject,details) VALUES (?,?,?,?,?)", (timestamp(), category, action, subject, json.dumps(details or {}, ensure_ascii=False, sort_keys=True)))
