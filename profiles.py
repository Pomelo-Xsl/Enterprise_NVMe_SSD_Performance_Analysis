...
CATALOGUE = {
    ...
    "sustained-write": BenchmarkProfile(
        "sustained-write",
        "持续顺序写",
        "识别缓存耗尽和稳态写入性能",
        "write",
    ...
    "recovery-probe": BenchmarkProfile(
        "recovery-probe",
        "恢复能力探测",
        "压力测试后的低风险恢复采样",
        "write",
...
def safe_fio_preview(profile: Mapping, device: str) -> list[str]:
    ...
    return [
        "fio",
        "--name=nvme-insight-preview",
        f"--filename={device}",
        f"--rw={profile['rw']}",
        f"--bs={profile['block_size']}",
        f"--iodepth={profile['io_depth']}",
        f"--numjobs={profile['jobs']}",
        f"--runtime={profile['runtime']}",
        f"--ramp_time={profile['ramp_time']}",
        "--direct=1",
        "--output-format=json+",
    ]
